...
网关启动时从配置端口开始探测：若 HTTP 或 SOCKS5 端口被其他程序占用，
会整体向后顺延寻找一对空闲端口（HTTP 与 SOCKS5 端口保持相邻偏移），
保证网关总能启动成功。实际绑定的端口通过 `/api/status` 的
`httpProxyBind` / `socks5ProxyBind` 字段返回，前端界面据此显示真实地址。
