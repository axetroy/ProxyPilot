...
> **合规边界**：默认仅监听本机（`127.0.0.1:17891`）。向他人共享节点订阅可能
> 违反所在司法辖区的法律（未经许可提供代理/VPN 服务），改为对外监听前需自行
> 评估合规风险并承担相应责任。
...
- 解析时机：节点检测完成时（`pool.evalOne`）解析并写入 `proxy_nodes`
  ...
  优先级：连接安全探测成功时的**节点出口 IP**（`ProxiedIP`，最准确，代表真实代理
...
6. **chain 代理链路**：客户端依次经过多个代理节点到达目标（客户端 → n0 → n1 → … → target），
   提升连接安全与多跳中继。链路由用户配置（`proxy_chains` 表，有序节点 ID 列表，可按需启用多条）。
...
| IP 级判断 | 可用 geoip 离线库（纯 IP 目标也能判） | 只能域名规则，IP 需 dnsResolve（受本地解析异常影响） |
...
直连需代理域名会因 DNS 解析异常解析到假 IP 而失败；反之即使代理名单误伤（把可直连域名
...
- **直连路径**：网关用本机 DNS 解析。命中直连的均为大陆 / 局域网 / 未需代理域名，
  本机 DNS 解析正常，无异常干扰。
...
| `pac_proxy_urls` | 上游代理名单主源+镜像 | 逗号分隔，按序尝试 |
