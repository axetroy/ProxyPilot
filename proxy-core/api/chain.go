...
func (s *Services) testChain(c *gin.Context) {
	...
	// 把手动测试结果同步到健康状态字段（界面展示用）；不动连续失败计数，
	// 自动停用判定仍由链路健康管理（chainhealth）按周期探测驱动。
	errMsg := ""
	if !result.OK {
		errMsg = validator.ChainError(result)
	}
	_ = s.Store.RecordChainCheck(id, result.OK, result.TotalLatency, errMsg)
